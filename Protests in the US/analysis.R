...
log_attendees <- log(num_attendees)
log_attendees_distribution <- boxplot(log_attendees)
...
prop_in_wa <- num_in_wa / length(num_locations)
...
count_in_location <- function(location){
  num_in_location <- sum(stringr::str_detect(locations, location), na.rm = TRUE)
  sentence <- paste("There were ", num_in_location, " protests in ", location)
  return(sentence)
}
...
# `stringr` package
states <- stringr::str_sub(protests$Location,nchar(protests$Location)-1, nchar(protests$Location))
...
in_2019 <- dates[ dates > '2019-1-01' & dates < '2020-1-01']

# What is the ratio of the number of protests in 2020 comparted to 2019?
# `ratio_2020_2019`
total_amount <- length(dates)
ratio_2020_2019 <- length(in_2020) / total_amount - length(in_2019)/ total_amount
...
count_on_date <- function(date){
  num_of_protests <- length(dates[dates == date])
  return(paste0("There were ", num_of_protests, " on ", date, " where "
               , num_of_protests, 
               " is the number of protests on that date. and ", 
               date, "is the date provided."))
}

# Using your function you just wrote, how many protests were there on
# May 24th, 2020? `num_may_24`
num_may_24 <- count_on_date('2020-5-24')

# Using your function you just wrote, how many protests were there on
# May 31th, 2020? `num_on_may_31`
num_on_may_31 <- count_on_date('2020-5-31')
...
months_for_2020 <- months(in_2020)
by_month_table <- table(months_for_2020)
...
months_for_2019 <- months(in_2019)
table_for_2019 <- table(months_for_2019)
...
table_of_purpose <- table(purpose)
View(table_of_purpose)
high_level_purpose <- gsub("\\s*\\([^\\)]+\\)","",as.character(unique_purposes))
...
number_of_high_level_purpose <- function(purpose){
  num_high_level <- sum(stringr::str_detect(high_level_purpose, purpose))
  sentence <- paste("There were", num_high_level, "types of protests for", purpose)
  return(sentence)
}